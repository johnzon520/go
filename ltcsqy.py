...
def capture_output():
    ...
    # 将 stdout 和 stderr 重定向到 StringIO 对象
...
# 将捕获的输出分开并发送到 push
def send_output_to_push(stdout, stderr):
    # 获取标准输出和标准错误的内容
    ...
    # 发送标准输出和标准错误输出
    if output:
        push("联通权益超市",f"{output}")
    
    if error_output:
        push("联通权益超市",f"{error_output}")
...
with capture_output() as (stdout, stderr):
    asyncio.run(main(ltTokens_list))
    send_output_to_push(stdout, stderr)
